...
| **M2** | 에디터 커스텀 툴 (핵심 어필) | 🔴 | 🟡 진행중(M2-1~2-4✅, 다음 M2-5) | [backlog-M2.md](backlog-M2.md) | SO DB + 적 조합 오서링 + 유효성 경고 + Addressables + 스폰 연결 |
...
| 2026-08-20 | **M2-4 완료(`[x]`, a~e)** — 유효성 경고(테이블→검증 툴 격상). R1·R2=`VD.Core.EnemyValidation.Validate`→`EnemyWarning` 리스트, R3(비주얼 `archetype:` 라벨 교차)=`VD.Editor`의 `AppendLabelWarning`(에디터 전용 Addressables API라 Editor층 불가피). 표시=경고박스+모순 필드 red(`.so-field-error`)+목록 행 ⚠, 실시간. **차단 아님(비차단 저장)**. e=창 실측(사용자 육안) 통과. **통합 기술문서 `06`은 미작성 폐기** — 한 번 썼으나 진행로그 톤이라 backlog-M2 상세 누적으로 대체(정식 기술문서는 후속 정리). 다음 = M2-5(툴 데이터→런타임 스폰). |
