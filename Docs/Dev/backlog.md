...
| 2026-08-22 | **오서링 툴 CSV export/import**(밸런싱 편의) — 베이스 `SoTableEditorView<T>`에 추가 → 4개 툴 공통. 숫자·bool·enum 리프(+중첩 struct 스칼라)만 평탄화(배열·오브젝트참조·`AssetReference`·문자열 제외), 이름 매칭 업데이트 전용. 스프레드시트 밸런싱 워크플로우. 상세 = [06_EnemyPipeline.md](06_EnemyPipeline.md) §2.1. |
