...
| 2026-08-19 | **M1-4 마감(`[x]`)** — 적 기본 엔티티 & 스폰. DoD(스폰·직진 접근·사격 파괴·충돌 HP감소·레이어 분리) 충족 상태로 마감. 남았던 "잔존"은 (a) 원뿔 각도/사거리 = **사용자 주도 튜닝**(제외), (b) `[TEMP]` 로그·조준 기즈모 정리 = **로그 유지 방침으로 취소**(기즈모 포함 그대로 둠) → 마감 블로커 아님. 산출·구조는 M1-4 상세 참조. |
