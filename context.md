...
- **✅ M5 진행(2026-08-22, 순서=M5-9→5-5→5-10)**: **M5-9 타이틀 씬**(`TitleController`, 배경/로고/BEST/3버튼, 타이틀→게임→결과→타이틀 루프 성립) · **M5-5 사운드/BGM**(`AudioManager` 싱글톤·프리팹 + `GameAudioMixer`[Master→BGM/SFX 노출볼륨] + `SfxId` 13종 3D/2D 배선 + `DamageSource` 피격 분기 + 적 사망 VFX·빈보이스 재생, 기술문서 [07_AudioSystem.md](Docs/Dev/07_AudioSystem.md)) · **M5-10 설정창**(`SettingsPanel` 프리팹=Sci-Fi UI 슬라이더3+%+PlayerPrefs, 노출=타이틀 "환경설정"+인게임 좌하단 기어[열면 일시정지], `SettingsOpener`). 상세=[backlog-M5.md](Docs/Dev/backlog-M5.md) ⚡.
- **▶ 밸런싱 패스 진행 중** — **작업 배치 상세·체크리스트 = [backlog-balance-pass.md](Docs/Dev/backlog-balance-pass.md)**(자체 포함: ①조이스틱 ②튜토리얼 ③웨이브/적/스폰/Orb/플레이어 스케일). **✅ ① 조이스틱 완료(2026-08-22)** — 포인터 이동을 **방향형 플로팅 조이스틱**으로 전환(상대 드래그 폐기, 스틱 유지=계속 이동). 가상 스틱 단일 소스 `VD.UI.JoystickView`(입력+시각, `Value`)를 `PlayerMovement`가 읽음. **PC 스탠드얼론=마우스 이동 차단·키보드만**(에디터/모바일만 조이스틱 활성). 조작 설계 [controls-design.md](Docs/Designs/controls-design.md) §3 갱신·기술문서 [03_PlayerMovementAndCamera.md](Docs/Dev/03_PlayerMovementAndCamera.md) 갱신 콜아웃. **다음 = ② 튜토리얼**. + **M4-8 플레이 검증**. 이후 M5 잔여(M5-4 무기 Lv5·M5-3 데미지넘버·M5-8 포메이션·M5-7 Firebase·M5-1 빌드[사용자]·M5-2 데모영상[사용자]).
